...
def test_replacer_applies_indexed_replacements_to_text():
    ...
    assert result == "Contact: [Phone_1]; email [Email_1]"


def test_detector_manager_uses_single_ner_pass():
    ...
    assert any(item["category"] == "PERSON" for item in results)
    assert any(item["category"] == "ORG" for item in results)
